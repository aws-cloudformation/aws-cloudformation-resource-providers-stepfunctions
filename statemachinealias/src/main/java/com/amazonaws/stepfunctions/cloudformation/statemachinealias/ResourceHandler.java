...
public abstract class ResourceHandler extends BaseHandler<CallbackContext> {
    ...
    if (e instanceof AmazonServiceException) {
      ...
      if (errorStatus >= 400 && errorStatus < 500) {
        ...
        if (Constants.THROTTLING_ERROR_CODE.equals(errorCode)) {
          ...
          resultBuilder.message(amznException.getMessage());
          resultBuilder.status(OperationStatus.FAILED);
        } else if (Constants.INVALID_REQUEST_ERROR_CODES.contains(errorCode)) {
          ...
          resultBuilder.status(OperationStatus.FAILED);
            ...
            resultBuilder.message(amznException.getMessage());
            resultBuilder.status(OperationStatus.FAILED);
        } else if (Constants.STATE_MACHINE_ALIAS_ALREADY_EXISTS.equals(errorCode)) {
  ...
  /**
   * Validates that the state machine alias ARN is not null
   ...
   */
  protected void verifyStateMachineAliasArnIsPresent(final String resourceArn) throws AmazonServiceException {
    if (resourceArn == null) {
      AmazonServiceException exception = new AmazonServiceException(Constants.RESOURCE_NOT_FOUND_ERROR_CODE);
  ...
  protected AmazonServiceException getStateMachineAliasAlreadyExistException() {
    final AmazonServiceException exception = new AmazonServiceException(Constants.STATE_MACHINE_ALIAS_ALREADY_EXISTS);
    exception.setStatusCode(400);
    exception.setErrorCode(Constants.STATE_MACHINE_ALIAS_ALREADY_EXISTS);
    return exception;
  }

  public static ValidationException getValidationException(final String message) {
      final ValidationException exception = new ValidationException(message);
      exception.setStatusCode(400);
      exception.setErrorCode(Constants.VALIDATION_ERROR_CODE);
      return exception;
  }
}
